            ...
            <div v-else class="doctors-list">
              ...
              >
                <div class="doctor-info">
                  ...
                  <p class="contact">✉️ {{ doctor.email }}</p>
                  <p class="status">✅ Disponibil</p>
                </div>
...
export default {
  ...
  methods: {
    async loadDoctors() {
      ...
      try {
        // Încarcă doctorii din user_profiles cu rol 'doctor' și status NULL (aprobați)
        const { data: doctors, error } = await supabase
          ...
          `)
          .eq('rol', 'doctor')
          .is('status', null) // Doar doctori aprobați (status NULL)
          .order('nume_complet')
    ...
    selectDoctor(doctor) {
      ...
      // Aici poți adăuga logica pentru programare (placeholder)
      alert(`Selectat: ${doctor.nume_complet}\nEmail: ${doctor.email}\n\nFuncționalitatea de programare va fi implementată în viitor.`)
    },
