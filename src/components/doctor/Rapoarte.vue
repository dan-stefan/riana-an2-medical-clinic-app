  ...
  <div v-if="show" class="modal-overlay" @click="handleOverlayClick">
    <div class="modal-container" @click.stop>
      ...
      <div class="modal-body">
        ...
        <div v-else class="reports-content" ref="reportContent">
          <!-- PDF header info (hidden by default, shown only in PDF) -->
          <div class="pdf-header">
            <h1>📊 Raport Medical - {{ new Date().toLocaleDateString('ro-RO') }}</h1>
            <p>Generat de: Sistem Medical Digital</p>
          ...
          <!-- Tabs Navigation - hide in PDF -->
          <div class="tabs-navigation pdf-hide">
            <button 
              v-for="tab in tabs"
              :key="tab.id"
  ...
  methods: {
    ...
    async exportReport() {
      try {
        ...
        console.log('Se generează raportul PDF...')
        
        // Get the content to convert
        const element = this.$refs.reportContent
        
        if (!element) {
          console.error('Eroare: Conținutul pentru export nu a fost găsit.')
          alert('Eroare: Conținutul pentru export nu a fost găsit.')
          return
        }
        ...
        const options = {
          ...
          filename: `Raport_Medical_${new Date().toLocaleDateString('ro-RO').replace(/\./g, '_')}.pdf`,
          image: { 
            type: 'jpeg', 
            quality: 0.98 
          },
        ...
        // Generate and download PDF using the global html2pdf
        await window.html2pdf().set(options).from(element).save()
        
        console.log('Raportul PDF a fost generat cu succes!')
        alert('Raportul PDF a fost generat cu succes! 📄')
        
      } catch (error) {
        console.error('Error generating PDF:', error)
        alert('Eroare la generarea raportului PDF: ' + error.message)
      }
    },

    // Add notification methods for future use
    showNotification(message, type = 'info', duration = 5000) {
      // Simple console implementation - you can replace with a toast library
      console.log(`${type.toUpperCase()}: ${message}`)
      
      // For now, use simple alerts for errors/success
      if (type === 'error') {
      ...
      // Return a timestamp as ID for compatibility
      return Date.now()
    },
...
/* Force styles for PDF generation */
.reports-content {
