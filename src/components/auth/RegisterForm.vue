...
export default {
  ...
  methods: {
    async handleRegister() {
      ...
      if (result.success) {
        // Verifică tipul contului și afișează mesajul corespunzător
        if (this.formData.rol === 'doctor') {
          // Pentru doctori - verifică dacă au fost deconectați automat (status 'waiting')
          if (result.requiresApproval) {
            this.successMessage = '🏥 S-a trimis cererea de cont doctor către administrator. Încearcă logarea cu acest cont în următoarele 24h.'
            this.showLoginRedirect = true
            this.startCountdown()
          } else {
            // Doctor aprobat imediat (de obicei nu se întâmplă)
            this.successMessage = 'Cont doctor creat cu succes! 🎉'
          ...
          // Pentru pacienți - procesare normală
          this.successMessage = 'Cont creat cu succes! 🎉'
          
          // Verifică dacă utilizatorul este logat automat ȘI dacă status-ul permite accesul
          if (this.authStore.isLoggedIn && this.authStore.userProfile?.status === null) {
            console.log('🔄 User is logged in automatically with valid status, redirecting to dashboard...')
            ...
            console.log('🔄 User needs to login manually or has restricted status, redirecting to login form...')
            if (this.authStore.userProfile?.status !== null) {
              this.successMessage = 'Cont creat, dar necesită aprobare. Te poți loga după aprobare.'
            } else {
